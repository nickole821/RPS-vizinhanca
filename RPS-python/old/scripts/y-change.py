# -*- coding: utf-8 -*-
"""
Created on Thu Jun 26 12:58:40 2025
...
def get_neighbors(i, j, position_matrix, x, reference_matrix=None):
    ...
    if cell_value == 'Y':
        ...
        num_neighbors_O = 8 # Vizinhos de Moore para 'O'para 'O'

        desired_zy = int(x * num_neighbors_O)
        desired_zy = max(1, desired_zy) 
        ...
        while len(all_potential_neighbors_info) < desired_zy + 1 and current_delta <= max_search_delta:
        ...
        for k in range(min(desired_zy, len(all_potential_neighbors_info))):
        ...
        delta_O_moore = 1
        i_seq = (np.arange(i - delta_O_moore, i + delta_O_moore + 1) % size)
        j_seq = (np.arange(j - delta_O_moore, j + delta_O_moore + 1) % size)
...
output_dir = "C:/Unicamp/mestrado/simulacoes/RPS-python/RPS-python/output/y-change/zy32/"
