# -*- coding: utf-8 -*-
"""
Created on Wed Jun 11 14:58:23 2025
...
def get_neighbors(i, j, position_matrix, propO, reference_matrix=None):
    ...
    if cell_value == 'Y':
        neighbors_raw = position_matrix[np.ix_(i_seq, j_seq)].flatten()
        prop = np.mean(neighbors_raw == 'O')
        if prop < propO:
            delta = 3
            i_seq = (np.arange(i - delta, i + delta + 1) % size)
            j_seq = (np.arange(j - delta, j + delta + 1) % size)
...
def get_fitness_matrix(position_matrix, payoff_matrix, propO):
    size = position_matrix.shape[0]
    ...
    for i in range(size):
        for j in range(size):
            ...
            # Obtem vizinhos para cálculo de raio
            neighbors = get_neighbors(i, j, position_matrix, propO)
            if s1 == 'O':
                delta = 3
            elif s1 == 'Y':
                delta = 3 if np.mean(neighbors == 'O') < propO else 2
            else:
                delta = 2
    ...
    for i in range(size):
        for j in range(size):
            if intermediate_matrix[i, j] == 'E':
                neighbors_info = get_neighbors(i, j, position_matrix, propO)
...
def simulation(position_matrix, payoff_matrix, num_generations, prob_reproduce):
    ...
    for g in trange(1, num_generations + 1, desc="Gerações", leave=False):
        fitness_matrix = get_fitness_matrix(position_matrix, payoff_matrix, propO)
...
output_dir = "C:/Unicamp/mestrado/simulacoes/RPS-python/RPS-python/output/adaptative/prop03/p0"
